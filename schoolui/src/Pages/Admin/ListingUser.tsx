...
export const ListingUser: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className={`fixed left-0 ${NAVBAR_OFFSET} h-[calc(100vh-112px)] w-64 z-40`}>
        <Sidebar />
      </div>
      ...
      <main className="ml-64 pt-[160px] px-6 pb-10">
        <div className="mb-6 rounded-xl bg-gradient-to-r from-black to-yellow-600 p-6 text-white shadow-lg">
          ...
          <p className="text-sm text-yellow-200">Manage students, teachers & staff efficiently</p>
        </div>
        ...
        {loading && <div className="text-center py-6 font-semibold">Loading users...</div>}
        ...
        <div className="overflow-x-auto rounded-xl bg-white shadow-lg">
          <table className="w-full text-sm">
            ...
            <tbody>
              {visibleUsers.map((u) => (
                <tr key={u.username} className="border-t hover:bg-yellow-50 transition">
                  ...
                  <td className="px-4 py-3 text-center">{u.isStaff ? "Yes" : "No"}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <select
                      className="rounded-md border px-2 py-1 text-sm bg-gradient-to-r from-black to-yellow-600 text-white"
                      onChange={(e) => {
                        if (e.target.value === "update") console.log("Update:", u.username);
                        if (e.target.value === "delete") console.log("Delete:", u.username);
                        e.target.value = "";
                      }}
                    >
